...
class RemotePmacInterface:
    ...
    PMAC_errors = {
        ...
        "ERR018": "Attempt to perform phase reference during move, "
                  ...
                  "error.",
        "ERR019": "Illegal position-change command while moves stored in "
                  "CCBUFFER"
    }
    ...
    def __init__(self, parent=None, verbose=False, numAxes=None, timeout=3.0):
        ...
        # These also need to be cached, as dls_pmaccontrol repeatedly polls
        ...
        self.MACRO_STATION_LOOKUP_TABLE = [0, 1, 4, 5, 8, 9, 12, 13, 16, 17, 20,
    ...
    def checkAxisIsInRange(self, axis):
        ...
        if axis > self.getNumberOfAxes():
            raise ValueError('Requested axis %d but PMAC has only %d axes' % (
    ...
    def getIVars(self, base, offsets):
        ...
        if status:
            ...
            raise IOError(
                'Cannot retrieve variable: error communicating with PMAC')

    def setVar(self, varName, value):
        (returnStr, wasSuccessful) = self.sendCommand(
            '%s=%s' % (varName, str(value)))
        if not wasSuccessful:
    ...
    def jogInc(self, motor, direction, distance):
        if direction == "neg":
            ...
            return "Error, could not recognise direction: " + str(
                direction), False
    ...
    def testGetAxisMacroStationNumber(self):
        for i in range(1, 33):
            try:
                print('pmac.getAxisMacroStationNumber(%d) returns %s.' % (
                    i, repr(self.getAxisMacroStationNumber(i))))
            except Exception as e:
                print('pmac.getAxisMacroStationNumber(%d) raised %s' % (
                    i, str(e)))

    def testIsMacroStationAxis(self):
        for i in range(1, 33):
            try:
                print('pmac.isMacroStationAxis(%d) returns %s.' % (
...
class PmacEthernetInterface(RemotePmacInterface):
    ...
    def connect(self, updatesReadyEvent=None):
        ...
        try:
            if self.verboseMode:
                print('Connecting a socket to host "%s" using port %d' % (
                    self.hostname, self.port))
            self.sock.connect((self.hostname, self.port))
            if self.verboseMode:
                print('Connected to host "%s" on port %d' % (
                    self.hostname, self.port))
    ...
    def _sendCommand(self, command, shouldWait=True, doubleTimeout=False):
        ...
        def getbufferRequest():
            request = struct.pack('8B', 0xC0, 0xC5, 0x0, 0x0, 0x0, 0x0, 0x08,
                                  0x0)  # 0x08,0x0 for a length of 2048; 1400
            # would be 0x05,0x78
            return request
            ...
            while True:
                try:
                    return self.sock.recv(
                        char_num)  # wait for and read the response from PMAC
...
class PmacTelnetInterface(RemotePmacInterface):
    ...
    def _sendCommand(self, command, shouldWait=True, doubleTimeout=False):
        ...
        if returnMatchNo == -1:
            raise IOError(
                'Timed out waiting for expected response. Got only: "%s"' %
                returnStr)
        else:
...
class PmacSerialInterface(RemotePmacInterface):
    ...
    def connect(self, updatesReadyEvent=None):
        ...
        try:
            self.serial = serial.Serial(self.comm_port, self.baud_rate,
                                        timeout=self.timeout, rtscts=True,
                                        dsrdtr=True)
        ...
        try:
            try:
                ...
                if time.time() - read_start > messageTimeout:
                    ...
                    print(
                        'Warning: Communication Timeout! (#%s)' %
                        self.n_timeouts)

                if returnMatchNo == [0]:
                    print("PMAC returned error: %s (%s)" % (
                    ...
                    if not returnMatchNo:
                        ...
                        print('Received: %r (total duration %s seconds, %s)' % (
                            returnStr, str(time.time() - comms_start),
                            returnMatchNo))
